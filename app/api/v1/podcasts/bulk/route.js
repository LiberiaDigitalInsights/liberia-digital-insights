...
async function bulkHandler(request) {
  try {
    ...
    let dbError;
    if (action === "delete") {
      const { error } = await supabase.from("podcasts").delete().in("id", ids);
      dbError = error;
    } else {
      const statusMap = {
        publish: "published",
    ...
    try {
      ...
      await logAudit({
        action: `BATCH_${action.toUpperCase()}_PODCASTS`,
        targetType: "podcast",
      ...
      console.error("[BulkPodcasts] Audit logging failed:", auditErr);
    }

    return NextResponse.json({
      message: `Successfully performed ${action} on ${ids.length} podcasts`,
      item_count: ids.length,
    });
  } catch (error) {
    console.error("[BulkPodcasts] error:", error);
