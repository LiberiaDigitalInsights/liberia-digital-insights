...
const __dirname = dirname(dirname(__filename)); // Go up one level to project root
...
async function killPort(port) {
  ...
  if (inUse) {
    colorLog(
      "yellow",
      `⚠️  Port ${port} is in use. Killing existing process...`,
    );
...
async function startDev() {
  colorLog(
    "cyan",
    "🚀 Starting Liberia Digital Insights Development Servers...",
  );
  colorLog("cyan", "==================================================");
  ...
  try {
    ...
    if (!fileExists(join(__dirname, "backend", ".env"))) {
      ...
      colorLog(
        "yellow",
        "Please copy backend/.env.example to backend/.env and configure your Supabase credentials.",
      );
      process.exit(1);
    }
    ...
    if (!fileExists(join(__dirname, "frontend", ".env"))) {
      ...
      colorLog(
        "yellow",
        "Please copy frontend/.env.example to frontend/.env and configure your environment variables.",
      );
      process.exit(1);
    }
    ...
    if (backendProcess.pid) {
      colorLog(
        "green",
        `✅ Backend server started (PID: ${backendProcess.pid})`,
      );
    ...
    if (frontendProcess.pid) {
      colorLog(
        "green",
        `✅ Frontend server started (PID: ${frontendProcess.pid})`,
      );
