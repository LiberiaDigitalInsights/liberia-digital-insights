...
router.post("/", async (req, res) => {
  try {
    ...
    if (catError) {
      ...
      return res.status(500).json({ error: "Failed to seed categories", details: catError });
    }
    ...
    const { data: articles, error: artError } = await supabase
      ...
      .upsert([
        ...
        },
        {
          ...
          excerpt: "How Liberia is embracing digital technologies to drive economic growth and innovation.",
    ...
    if (artError) {
      ...
      return res.status(500).json({ error: "Failed to seed articles", details: artError });
    }
    ...
    if (podError) {
      ...
      return res.status(500).json({ error: "Failed to seed podcasts", details: podError });
    }
    ...
    if (advError) {
      ...
      return res.status(500).json({ error: "Failed to seed advertisements", details: advError });
    }
