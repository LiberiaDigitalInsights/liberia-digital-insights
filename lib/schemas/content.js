...
export const podcastSubmissionSchema = z.object({
  ...
  status: z.enum(["draft", "published", "archived"]).default("draft"),
  tags: z.array(z.string()).optional(),
  // Legacy plain guest name (kept for backwards compat)
  ...
  guest_profile: z
    ...
    .optional(),
  // Episode metadata
  season_number: z.number().int().positive().optional().nullable(),
  episode_number: z.number().int().positive().optional().nullable(),
  ...
  apple_url: z
    .string()
    .url("Invalid Apple Podcasts URL")
    .optional()
    .or(z.literal("")),
});
...
export const bookmarkSchema = z.object({
  ...
  content_type: z.enum(["article", "event", "insight", "podcast"]),
});

export const bookmarkQuerySchema = paginationSchema.extend({
  content_type: z.enum(["article", "event", "insight", "podcast"]).optional(),
  resolve: z.string().optional(),
});

// Bulk operation schema for batch actions
