          ...
          <>
                ...
                {Array.isArray(tags) && tags.length > 0 && (
                  <span className="inline-flex flex-wrap items-center gap-1">
                    {tags.slice(0, 2).map((t) => (
                      <span
                        key={t}
                        className="rounded-full bg-[color-mix(in_oklab,var(--color-surface),white_10%)] px-2 py-0.5 text-[10px] font-medium text-muted"
                      >
                        #{String(t).replace(/^#/, '')}
                      </span>
                    ))}
            ...
            {image && (
              ...
              >
                ...
                <div className="absolute inset-0 bg-linear-to-t from-black/20 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100 pointer-events-none" />
                {category && (
